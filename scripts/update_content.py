...
def _md_list(items: list[dict], extra_key: str | None = None) -> str:
    lines = []
    for it in items:
        link  = it.get("link") or ""
        ...
        meta  = " · ".join(p for p in [date, extra] if p)
...
def write_summary(buckets: dict[str, list[dict]], queue_counts: dict[str, int],
    ...
    lines = [
        f"## {kind} — {run_date}",
        f"",
        f"**{total} new item(s).**  Review each entry, then merge to publish — or close to discard.",
        f"",
        f"---",
        f"",
        ...
        f"",
        f"**News** ({len(q_news)})",
        _md_list(q_news, extra_key="type"),
        f"",
        f"**Media** ({len(q_media)})",
        _md_list(q_media, extra_key="outlet"),
        f"",
        f"**Articles** ({len(q_articles)})",
        _md_list(q_articles, extra_key="source"),
        f"",
        f"---",
        f"",
        ...
        f"---",
        f"",
        f"_Auto-generated by `scripts/update_content.py`. Links and descriptions may need manual review._",
    ]
